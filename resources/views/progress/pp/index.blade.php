...
<!-- Page Content -->
<div class="content">
	<h2 class="content-heading">Pra Preparation Process (PP)</h2>
	<!-- Dynamic Table Full -->
	<div class="block">
		<div class="block-header block-header-default">
			...
			<select class="js-select2 form-control" id="proc-filter" style="width: 20%;">
				...
				@for ($i=0; $i < count($_PpProcDats['proc_id']); $i++)
					<option value="{{ $_PpProcDats['proc_name'][$i] }}"> {{ $_PpProcDats['proc_name'][$i] }} </option>
				@endfor
			</select>
			&nbsp;
			...
			&nbsp;&nbsp;
			@if(DB::table('tb_material')->select('pp_stat_id')->where('proj_id',$proj_id)->where('pp_stat_id','>',1)->exists())
			<a href="{{ route('pp.allqrcode',$proj_id) }}" target="_blank">
			...
			@if (Auth::user()->roles->first()->name == 'Production Planning Control' || Auth::user()->roles->first()->name == 'Super Admin')
				<a href="{{ route('pp.create', $proj_id) }}" data-remote="false" data-toggle="modal" data-target="#formModal">
					<button type="button"name="add_material" id="add_material" class="btn btn-sm btn-rounded btn-noborder btn-primary"><i class="fa fa-plus text-primary-dark"></i>&nbsp;&nbsp;Add material</button>
				</a>
			@endif
		</div>
