			...
			<div class="col-xl-10">
				...
				<form method="POST" id="material_form" name="material_form" class="js-validation-bootstrap form-horizontal" enctype="application/x-www-form-urlencoded">
						...
						<div class="col-lg-8">
							<select class="form-control" id="proc_id" name="proc_id">
								@for ($i=0; $i < count($_PrProcDats['proc_id']); $i++)
									@if ($_PrProcDats['proc_id'][$i] < $mat_dats->pr_proc_id)
										@continue
									@endif
									<option value="{{ $_PrProcDats['proc_id'][$i] }}" {{ $_PrProcDats['proc_id'][$i] == $mat_dats->pr_proc_id ? 'selected' : '' }}> {{ $_PrProcDats['proc_name'][$i] }} </option>
								@endfor
							</select>
						</div>
					</div>
					...
					<div class="form-group row">
						...
						<div class="col-lg-8">
							<input readonly type="number" step="1" class="form-control" id="mat_no" name="mat_no" placeholder="Material Weight" value="{{$mat_dats->mat_no}}">
						</div>
					</div>
					<div class="form-group row">
						<label class="col-lg-4 col-form-label" for="mat_spec">Specification</label>
						<div class="col-lg-8">
						<input readonly type="text" class="form-control" id="mat_spec" name="mat_spec" placeholder="Material Specification" value="{{$mat_dats->mat_spec}}">
						</div>
					</div>
					<div class="form-group row">
						<label class="col-lg-4 col-form-label" for="mat_thick">Thickness</label>
						<div class="col-lg-8">
							<input readonly type="number" step="0.0001" class="form-control" id="mat_thick" name="mat_thick" placeholder="Material Thickness" value="{{$mat_dats->mat_thick}}">
						</div>
					</div>
					<div class="form-group row">
						<label class="col-lg-4 col-form-label" for="purchased_at">Purchased At</label>
						<div class="col-lg-8">
							<input readonly type="date" class="form-control" id="purchased_at" name="purchased_at" placeholder="date" @if(!is_null($mat_dats->purchased_at)) value="{{$mat_dats->purchased_at}}" @endif>
						</div>
					</div>
					<div class="form-group row">
							<label class="col-lg-4 col-form-label" for="arrived_at">Arrived At</label>
							<div class="col-lg-8">
							<input readonly type="date" class="form-control" id="arrived_at" name="arrived_at" placeholder="date" @if(!is_null($mat_dats->arrived_at)) value="{{$mat_dats->arrived_at}}" @endif>
							</div>
						</div>
					<div class="form-group row">
					...
					<div class="form-group row">
						<div class="col-lg-8 ml-auto">
							<input type="hidden" name="proc_old" id="proc_old" value={{ $mat_dats->pr_proc_id }} />
...
$('#material_form').on('submit', function(event){
	...
	$.ajax({
		url:"{{ route('pr.update', [$proj_id, $pr_id]) }}",
		method:"POST",
